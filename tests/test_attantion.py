...
def test_softmax_multidimensional():
    ...
    expected = jnp.array(
        [[0.26894143, 0.7310586], [0.26894143, 0.7310586], [0.26894143, 0.7310586]]
    )
    np.testing.assert_allclose(softmax(x), expected, rtol=1e-6)
...
def test_attention_basic():
    ...
    K = jnp.array(
        [[50.0, 0.0], [0.0, 50.0]]
    )  # Increased values to push softmax to extremes
    V = jnp.array([[50.0, 0.0], [0.0, 50.0]])

    expected = V  # in this specific setup, output should be very close to V
    np.testing.assert_allclose(
        scaled_dot_product_attention(Q, K, V), expected, atol=1e-1, rtol=1e-1
    )  # increased tolerance
